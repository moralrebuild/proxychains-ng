...
extern int whois_quiet_mode;
extern unsigned int whois_proxy_offset;
...
static int tunnel_to(int sock, ip_type ip, unsigned short port, proxy_type pt, char *user, char *pass) {
	...
	if(ulen > 0xFF || passlen > 0xFF || dns_len > 0xFF) {
		whois_write_log(LOG_PREFIX "error: maximum size of 255 for user/pass or domain name!\n");
		goto err;
	}
	...
	switch (pt) {
		case HTTP_TYPE:{
			if(!dns_len) {
				if(!inet_ntop(v6?AF_INET6:AF_INET,ip.addr.v6,ip_buf,sizeof ip_buf)) {
					whois_write_log(LOG_PREFIX "error: ip address conversion failed\n");
					goto err;
				}
				dns_name = ip_buf;
			}
		...
		case SOCKS4_TYPE:{
			if(v6) {
				whois_write_log(LOG_PREFIX "error: SOCKS4 doesn't support ipv6 addresses\n");
				goto err;
			}
...
static int start_chain(int *fd, proxy_data * pd, char *begin_mark) {
	...
	*fd = socket(v6?PF_INET6:PF_INET, SOCK_STREAM, 0);
	...
	whois_write_log(LOG_PREFIX "%s " TP " %s:%d ",
...
static int chain_step(int ns, proxy_data * pfrom, proxy_data * pto) {
	...
	usenumericip:
		if(!inet_ntop(v6?AF_INET6:AF_INET,pto->ip.addr.v6,ip_buf,sizeof ip_buf)) {
			pto->ps = DOWN_STATE;
			whois_write_log("<--ip conversion error!\n");
			close(ns);
			return SOCKET_ERROR;
		}
		hostname = ip_buf;
	...
	whois_write_log(TP " %s:%d ", hostname, htons(pto->port));
	...
	switch (retcode) {
		...
		case BLOCKED:
			...
			whois_write_log("<--denied\n");
			close(ns);
			break;
		case SOCKET_ERROR:
			pto->ps = DOWN_STATE;
			whois_write_log("<--socket error or timeout!\n");
			close(ns);
			break;
	}
	return retcode;
...
int connect_proxy_chain(int sock, ip_type target_ip,
	...
	switch (ct) {
		...
		case ROUND_ROBIN_TYPE:
			alive_count = calc_alive(pd, proxy_count);
			curr_pos = offset = whois_proxy_offset;
			...
			p3->ip = target_ip;
			p3->port = target_port;
			whois_proxy_offset = offset+1;
			PDEBUG("pd_offset = %d, curr_len = %d\n", whois_proxy_offset, curr_len);
			if(SUCCESS != chain_step(ns, p1, p3))
				goto error;
			break;
